<section class="group_top">
    <section class="top">
        <div class="presentation_top">
            <h1>La Grand-Est</h1>
            <p>Découvrez les saveurs du miel et du sapin à travers cette bière.</p>
            <p>Pour plus de plaisir, associez cette bière à un beau morceau de Munster venu tout droit du Grand Est !</p>
        </div>
        <div class="photo_top">
            <img src="images/biere_accueil.png">
        </div>  
    </section>
    <div class="bouton_place">
        <a href="#" id="bouton_decouvrir">Découvrir</a>
    </div>
</section>


<section class="publicite">
    <h2>Trouvez la parfaite osmose entre nos bières aux arômes surprenants et vos fromages régionaux de qualité supérieure !</h2>
    <video class="publicite_osmose" controls>
        <source src="videos/publicite_osmose.mov" type="video/mov">
    </video>
</section>

<section class="plus_vendues">
    <div class="text_plus_vendues">
        <h1>Les Plus Vendues</h1>
    </div>
    <div class="bieres">
        <div class="bloc_biere">
            <img src="images/grand_est.png">
            <div class="text_bieres">
                <h3>La Grand-Est</h3>
                <h5>4,90 €</h5>
            </div>
        </div>
        <div class="bloc_biere">
            <img src="images/normandie.png">
            <div class="text_bieres">
                <h3>La Normande</h3>
                <h5>4,90 €</h5>
            </div>
        </div>
        <div class="bloc_biere">
            <img src="images/occitanie.png">
            <div class="text_bieres">
                <h3>L'Occitane</h3>
                <h5>4,90 €</h5>
            </div>
        </div>
        <div class="bloc_biere">
            <img src="images/corse.png">
            <div class="text_bieres">
                <h3>La Corse</h3>
                <h5>4,90 €</h5>
            </div>
        </div>
    </div>
</section>


<section class="nos_box">
    <div class="text_nos_box">
        <h1>Les Box</h1>
    </div>
    <div class="box">
        <div class="bloc_box">
            <img src="images/box_grand_est.png">
            <div class="text_box">
                <h3>Grand-Est</h3>
                <h5>11,90 €</h5>
            </div>
        </div>
        <div class="bloc_box">
            <img src="images/box_normandie.png">
            <div class="text_box">
                <h3>Normandie</h3>
                <h5>11,90 €</h5>
            </div>
        </div>
        <div class="bloc_box">
            <img src="images/box_occitanie.png">
            <div class="text_box">
                <h3>Occitanie</h3>
                <h5>11,90 €</h5>
            </div>
        </div>
        <div class="bloc_box">
            <img src="images/box_corse.png">
            <div class="text_box">
                <h3>Corse</h3>
                <h5>11,90 €</h5>
            </div>
        </div>
    </div>
</section>